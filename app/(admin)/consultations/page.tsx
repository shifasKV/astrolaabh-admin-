  ...
  return (
    <>
      ...
      >
        ...
        {/* Filter strip — filters left, search + sort right (hidden in calendar view) */}
        {viewMode === "list" && <div className="hidden sm:flex flex-wrap items-center gap-2 pt-4" style={{ borderTop: `1px solid ${T.borderSoft}` }}>
        ...
        return (
          <div className="flex items-start gap-4 md:flex-1 md:min-h-0">
            ...
            <Card className="md:hidden !p-0 overflow-hidden w-full">
              <MobileAgenda
                className=""
                events={MOCK_CONSULTATIONS.filter((c) => c.scheduledAt).map((c) => ({
                  id: c.id,
                  ...
                  title: c.customerName,
                  sub: `${c.expertName} · ${c.type.replace(/_/g, " ").replace(/^./, (ch) => ch.toUpperCase())}`,
                  color: eventTone(c),
                  href: `/consultations/${c.id}`,
                }))}
              />
