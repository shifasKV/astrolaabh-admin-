...
/* One seamless form surface — sections separated by hairlines, not floating cards */
...
/* Selectable chip — filled sage when active, subtle otherwise */
function ChipToggle({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    ...
    >
      {active && (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3"><path d="M20 6 9 17l-5-5" /></svg>
      )}
      {label}
    </button>
  );
...
export default function EditAstroGemologistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const validate = () => {
    const e: Record<string, string> = { name: V.required(name, "Full name"), email: V.email(email), phone: V.phone(phone), age: V.optionalNumber(age, "Age"), fee: V.positiveNumber(fee, "Session rate"), ifsc: V.ifsc(ifsc) };
    setErrors(e);
    return V.isClean(e);
  };
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-5 items-start pb-24">
            ...
            <div className="flex flex-col items-center text-center pb-4 mb-4" style={{ borderBottom: `1px solid ${T.borderSoft}` }}>
              <div className="relative">
                {photoPreview ? (
                  ...
                  <div className="w-[72px] h-[72px] rounded-[20px] flex items-center justify-center text-[24px] font-semibold" style={{ background: T.accentFaint, border: `1px solid ${T.accentBorder}`, color: T.accent }}>{(name || expert.name)[0]}</div>
                )}
              </div>
        ...
        <Card className="!p-0 overflow-hidden">
          ...
          <Section title="Commission & payout" sub="Commission percentages and the bank account for payouts.">
            <div>
      ...
      {/* Sticky action bar */}
      <div className="sticky bottom-0 -mx-5 md:-mx-10 px-5 md:px-10 py-3.5 flex items-center justify-end gap-2.5" style={{ background: "rgba(248,245,238,0.9)", backdropFilter: "blur(6px)", borderTop: `1px solid ${T.borderSoft}` }}>
