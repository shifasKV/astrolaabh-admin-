...
/**
 * Admin edit gemologist — layout matches expert My profile (identity rail + expertise + commission/payout).
 */
export default function EditAstroGemologistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [age, setAge] = useState(expert?.id === "usr_expert_01" ? "52" : expert?.id === "usr_expert_02" ? "44" : "");
  const [gender, setGender] = useState(expert?.id === "usr_expert_02" ? "female" : "male");
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-5 items-start pb-24">
        {/* Identity rail — same structure as expert My profile */}
        <aside className="lg:sticky lg:top-4">
          <Card className="!p-5">
            <div className="text-[11px] font-medium tracking-[0.08em] uppercase mb-4" style={{ color: T.faint }}>
              Identity
            </div>
            ...
            <div className="space-y-3.5 pt-4" style={{ borderTop: `1px solid ${T.borderSoft}` }}>
              <Input
                value={name}
                onChange={(v) => {
                  setName(v);
                ...
                onBlur={() => setErrors((p) => ({ ...p, name: V.required(name, "Full name") }))}
                error={errors.name}
                label="Full name"
                placeholder="e.g. Pt. Sandeep Kochaar"
              />
              <Input
                value={email}
                onChange={(v) => {
                ...
                onBlur={() => setErrors((p) => ({ ...p, email: V.email(email) }))}
                error={errors.email}
                label="Email"
                type="email"
                placeholder="e.g. name@astrolaabh.house"
              />
              <Input
                value={phone}
                onChange={(v) => {
                ...
                onBlur={() => setErrors((p) => ({ ...p, phone: V.phone(phone) }))}
                error={errors.phone}
                label="Mobile number"
                type="tel"
              ...
              <div className="grid grid-cols-2 gap-3">
                <Input
                  ...
                  placeholder="e.g. 45"
                />
                <Select value={gender} onChange={setGender} label="Gender" options={GENDER_OPTIONS} placeholder="Select" />
              </div>
            </div>
          </Card>
        </aside>
        ...
        <div className="space-y-4">
          <Card className="!p-6">
            <h2 className="text-[15px] font-semibold tracking-[-0.01em]" style={{ color: T.text }}>Expertise</h2>
            <p className="text-[12.5px] mt-1 mb-5" style={{ color: T.muted }}>
              Languages, specializations, experience and session rate.
            </p>
            <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {SKILL_OPTIONS.map((skill) => (
                    <ChipToggle
                      key={skill.value}
                      label={skill.label}
                      active={skills.includes(skill.value)}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Input
                  ...
                  onBlur={() => setErrors((p) => ({ ...p, fee: V.positiveNumber(fee, "Session rate") }))}
                  error={errors.fee}
              ...
              <Textarea
                ...
                onChange={setBio}
                label="Bio / About"
                placeholder="Brief description about the expert's background and expertise…"
                rows={3}
              />
            </div>
          </Card>
          ...
          <Card className="!p-6">
            ...
            <p className="text-[12.5px] mt-1 mb-5" style={{ color: T.muted }}>
              Percentages applied to stone, jewellery, and consultation earnings.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {(
                [
                  ["Stone", stoneRate, setStoneRate],
                  ["Jewellery", jewelleryRate, setJewelleryRate],
                  ["Consultation", consultationRate, setConsultationRate],
                ] as const
              ).map(([cat, val, setter]) => (
      ...
      <div
        className="sticky bottom-0 -mx-5 md:-mx-10 px-5 md:px-10 py-3.5 flex items-center justify-end gap-2.5"
