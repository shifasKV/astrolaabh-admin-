  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
        ...
        {/* Stepper */}
        {flowType === "stone" && <Stepper steps={["Choose Stone", "Choose Jewellery", "Generate Link"]} current={stoneStep} />}
      ...
      {flowType === "stone" && (
        <>
          ...
          {stoneStep === 1 && (
            <>
              {selectedStone && (
                <div className="rounded-[12px] px-4 py-3 mb-4 flex items-center gap-3" style={{ background: `${T.accent}08`, border: `1.5px solid ${T.accent}30` }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[12px] ml-2" style={{ color: T.muted }}>{selectedStone.sku} · {selectedStone.ratti} ratti · {catalogInr(selectedStone.price)}</span>
                  </div>
                  <GoldBtn className="!h-9 !px-5 !text-[12px]" onClick={() => setStoneStep(2)}>Next →</GoldBtn>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {stonePaged.map((s) => {
                  const isSelected = selectedStone?.sku === s.sku;
                  return (
                    <button
                      key={s.sku}
                      type="button"
                      onClick={() => setSelectedStone(s)}
                      className="rounded-[12px] p-3 text-left transition-all duration-200 cursor-pointer hover:-translate-y-0.5 hover:shadow-md relative"
                      style={{ background: isSelected ? `${T.accent}06` : T.card, border: `1.5px solid ${isSelected ? T.accent : T.border}` }}
                    >
                      {isSelected && (
          ...
          {stoneStep === 2 && (
            <>
              ...
              {selectedDesign && (
                <div className="rounded-[12px] px-4 py-3 mb-4 flex items-center gap-3" style={{ background: `${T.accent}08`, border: `1.5px solid ${T.accent}30` }}>
                  <div className="w-10 h-10 rounded-[8px] flex items-center justify-center text-[18px]" style={{ background: T.panel, border: `1px solid ${T.borderSoft}` }}>
                    {selectedDesign.form === "Ring" ? "💍" : selectedDesign.form === "Pendant" ? "📿" : "⌚"}
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="text-[13px] font-semibold" style={{ color: T.text }}>{selectedDesign.name}</span>
                    <span className="text-[12px] ml-2" style={{ color: T.muted }}>{selectedDesign.form} · {selectedDesign.metal}</span>
                  </div>
                  <button onClick={() => setSelectedDesign(null)} className="text-[12px] cursor-pointer hover:underline" style={{ color: T.accent }}>Remove</button>
                </div>
              )}
              ...
              <div className="flex flex-wrap items-center gap-2.5 mb-4">
                ...
                <div className="w-[140px]"><Select value={designMetal} onChange={(v) => { setDesignMetal(v); setDesignPage(0); }} compact options={METAL_OPTIONS} /></div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {designPaged.map((d) => {
                  const isSelected = selectedDesign?.slug === d.slug;
                  return (
                    <button
                      key={d.slug}
                      type="button"
              ...
              {/* Skip + campaign below grid */}
              <div className="mt-6 max-w-[500px] space-y-3">
                <div>
                  <div className="text-[11px] tracking-[0.08em] uppercase mb-2" style={{ color: T.faint }}>Campaign name (optional)</div>
                  <input
                    value={campaign}
                ...
                <GoldBtn className="!h-12 !px-8 !text-[14px] w-full" onClick={() => { setSelectedDesign(null); setStoneStep(3); }}>
                  Skip jewellery — generate as loose stone
                </GoldBtn>
              </div>
            </>
          )}
